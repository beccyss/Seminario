// Calculadora Básica: crie operações de soma, subtração, multiplicação,
// divisão, potência e raiz.

// gcc -O -Wno-unused-result calculadora.c -o main -lm
...
int main()
{
  ...
  else
  {
    ...
    if (operador == 6)
    {
      ...
      double resultado =
          pow(n, 1.0 / expoente); // Use a função pow() para calcular a raiz com
                                  // o expoente
    ...
    if (operador == 7)
    {
      ...
      printf("\nPara realizar o cálculo de potência é necessário que você "
             "insira dois valores para X e N. Onde X é a base e N o expoente.");
      printf("\n\nValor de X:");
      scanf("%d", &x);
      printf("\nValor de N:");
      scanf("%d", &n);
